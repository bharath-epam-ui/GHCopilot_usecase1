...
export default function DashboardPage() {
  ...
  async function handleTransition(id: string, nextStatus: TaskStatus) {
    const current = tasks.find((t) => t.id === id);
    if (!current) return;

    setTransitionState((s) => ({
      pendingById: { ...s.pendingById, [id]: true },
      errorById: { ...s.errorById, [id]: undefined },
    }));
    ...
    if (!res.ok) {
      ...
      const msg = json?.error?.message ?? json?.error ?? "Couldn't update status. Please try again.";
      setTransitionState((s) => ({
    ...
    const updated: Task | undefined = json?.data;
    if (updated) {
          ...
          <div className="grid gap-4 sm:grid-cols-2" data-testid="task-list">
            {tasks.map((task) => (
              <TaskCard
                ...
                isTransitioning={!!transitionState.pendingById[task.id]}
                transitionError={transitionState.errorById[task.id]}
              />
            ))}
          </div>
        )}
